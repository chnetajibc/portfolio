import { AI_CONFIG, getEnvConfig, isDailyLimitError, type Env } from "../config.js";
...
// Single canonical message builder — used by both streaming and non-streaming paths
export function buildChatMessages(userMessage: string): Array<{ role: string; content: string }> {
  // Static prefix first for prompt-prefix caching, dynamic user content last
  return [
...
export async function generateChatResponse(
  ...
  try {
    ...
    const msg = String((err as { message?: string })?.message || e || "");
